...
export interface ExecutionResult {
  status: 'success' | 'error' | 'timeout' | 'compilation_error' | 'runtime_error';
  output?: string;
  expectedOutput?: string;
...
class CodeExecutionService {
   ...
   */
  private async waitForCompletion(token: string, maxAttempts = 30): Promise<SubmissionStatus> {
  ...
  private async executeTestCase(
    ...
    try {
      // Submit code for execution
      const token = await this.submitCode(sourceCode, languageConfig.id, testCase.input);
  ...
  async executeCode(
    ...
    if (!this.apiKey || this.apiKey === 'your-rapidapi-key-here') {
      throw new Error('RapidAPI key not configured. Please set VITE_RAPIDAPI_KEY environment variable.');
    }
    ...
    for (const testCase of testCases) {
      ...
      if (result.errorMessage) {
        if (result.errorMessage.includes('compilation') || result.errorMessage.includes('syntax')) {
    ...
    return {
      ...
      compilationError: hasCompilationError ? compilationErrorMessage : undefined,
      errorMessage: hasRuntimeError ? 'Runtime error occurred during execution' : undefined
    };
  }
   ...
   */
  getAvailableLanguages(): Array<{ key: SupportedLanguage; name: string; extension: string }> {
   ...
   */
  validateSyntax(sourceCode: string, language: SupportedLanguage): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    switch (language) {
      case 'python':
        if (sourceCode.includes('import os') || sourceCode.includes('import subprocess')) {
      ...
      case 'cpp':
      case 'c':
        if (sourceCode.includes('#include <fstream>') || sourceCode.includes('#include <filesystem>')) {
