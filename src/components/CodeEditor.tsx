...
import { Play, RotateCcw, Send, Clock, AlertCircle, CheckCircle, XCircle, Loader } from 'lucide-react';
import { codeExecutionService, SupportedLanguage, TestCase, ExecutionResult } from '../services/codeExecutionService';
...
const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (code.trim()) {
        const validation = codeExecutionService.validateSyntax(code, selectedLanguage);
  ...
  const handleRunCode = async () => {
    ...
    try {
      const result = await codeExecutionService.executeCode(code, selectedLanguage, testCases);
  ...
  const handleSubmit = () => {
    if (executionResult && executionResult.testCasesPassed === executionResult.totalTestCases) {
      ...
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 p-4">
        ...
        {syntaxErrors.length > 0 && (
          <div className="flex items-center space-x-2 text-red-600 dark:text-red-400">
            ...
            <span className="text-sm">{syntaxErrors.length} syntax error(s)</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        {syntaxErrors.length > 0 && (
          <div className="border-t border-gray-200 dark:border-gray-700 bg-red-50 dark:bg-red-900/20 p-4">
            <h4 className="text-sm font-semibold text-red-700 dark:text-red-300 mb-2">Syntax Errors:</h4>
        ...
        {showResults && executionResult && (
          <div className="border-t border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-900">
            <div className="mb-4">
              <div className={`flex items-center space-x-2 p-3 rounded-lg border ${getStatusColor(executionResult.status)}`}>
                ...
                <div>
                  ...
                  {executionResult.executionTime && (
                    <div className="text-sm opacity-75">
                      Execution time: {executionResult.executionTime.toFixed(3)}s
                    </div>
                  )}
                </div>
              </div>
            ...
            {(executionResult.compilationError || executionResult.errorMessage) && (
            ...
            <div className="space-y-3">
              <h4 className="font-semibold text-gray-900 dark:text-white">Test Case Results:</h4>
                ...
                >
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    {result.executionTime && (
                      <span className="text-sm text-gray-500">({result.executionTime.toFixed(3)}s)</span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                    ...
                    <div>
                      <div className="font-medium text-gray-700 dark:text-gray-300">Expected:</div>
                      <pre className="bg-gray-100 dark:bg-gray-800 p-2 rounded text-gray-900 dark:text-gray-100 whitespace-pre-wrap">
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700">
          ...
          <button
            ...
            disabled={!executionResult || executionResult.testCasesPassed !== executionResult.totalTestCases}
            className={`flex items-center space-x-2 font-medium py-2 px-6 rounded-lg transition-colors duration-200 ${
              executionResult && executionResult.testCasesPassed === executionResult.totalTestCases
